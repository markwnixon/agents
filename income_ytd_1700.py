...
year, ytd_month, ytd_day = 2026, 6, 13
...
if success:
    ...
    for pdat in pdata:
        ...
        e3 = pdat.Associate2
        e4 = pdat.Company
        if hasinput(e1) or hasinput(e2) or hasinput(e3) or hasinput(e4):
    ...
    for cdat in cdata_all:
        type = cdat.Type
        con = cdat.Container
        if type == 'Load In' or type == 'Empty In' or type == 'Dray In':
    ...
    for cid in conid:
        ...
        if cjo is not None and cjo != 'NAY':
            ...
            if odat is not None:
                try:
                    ...
                    print(f'No invoice total for {odat.Jo}, shipper {odat.Shipper} showing {odat.InvoTotal}, so setting to zero for now')
                    oamt = 0.00
                sh = odat.Shipper
                sid = odat.id
                jo = odat.Jo
                ...
                rel = cdat.Release
                stat = cdat.Status
                if rel == 'DP OP' or rel == 'Dry Run Return' or stat == 'Work Exchange':
                    sid = 0
                    sh = 'Noncollect'
                    cjo = 'DP OP'
            ...
            try:
                oamt = 0.00
                rel = cdat.Release
                if rel is None: rel = 'None Found'
                stat = cdat.Status
                note = cdat.Other
                if rel == 'DP OP' or rel == 'Dry Run Return' or stat == 'Work Exchange' or 'Uncollectable' in note:
                    sh = 'Noncollect'
                    cjo = 'DP OP'
                    sid = 0
                    ...
                    if 'Uncollectable' in note:
                        issuecon2.append(['Not Listed', ccon, f'{cdate}', 0.00, 0.00, f'Uncollectable dry run'])
                else:
                    print(f'************************No job found for this container {ccon} with release {rel} and status {stat} ***************************')
                    sh = 'No Job'
                    cjo = 'No Job'
                    sid = 0
                    jo = 'None'
                ...
                print(f'Could not find information for container {ccon}')
        ...
        if istat == 4 or istat == 5 or istat == 8: pd_col[iport] += oamt
            ...
            if sh != 'Noncollect' and sh != 'FEL Ocean Div':
                pd_unc[iport] += oamt
                if sid == 0:
                    jo = None
    ...
    desc = f'Report last updated on {todaydate} for Year to Date: {ytd}'


    start_row = 3
    # write the headers
    headers = ['Month', 'Monthly Revenues', 'Cumulative Revenue', ' Cash Jobs ', 'Invoiced Jobs', 'Revenue Collected', 'Open Invoices', 'Port Days', 'Port-Truck-Days', 'Income/Port-Truck-Day']
    ...
    for jx, col in enumerate(headers):
        d = ws2.cell(row=1, column=jx + 1, value=col)
        d.font = Font(name='Calibri', size=10, bold=True)
        d.alignment = Alignment(horizontal='center')
    for invo in involist:
        ...
        for jx, item in enumerate(invo):
            ...
            d.font = Font(name='Calibri', size=10, bold=False)
            d.alignment = Alignment(horizontal='center')
            if flist[jx] != 0: d.number_format = flist[jx]

    column_widths = column_wide(headers, nopaycon, [])
    for i, column_width in enumerate(column_widths):
        ws2.column_dimensions[get_column_letter(i + 1)].width = column_width + 3
