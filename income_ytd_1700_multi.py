...
start_year = 2024
num_runs = 12
#ytd = datetime.date(start_year, start_month, start_day)
ytdvec = []
last_day_vec = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
for ix in range(12):
    #ytd = ytd + datetime.timedelta(days=14)
    start_month += 1
    ytd = datetime.date(start_year, start_month, last_day_vec[ix])
    ytdvec.append(ytd)
print(ytdvec)

big_jo_list = []

for yx, ytd in enumerate(ytdvec):

    year = ytd.year
    ytd_month = ytd.month
    ytd_day = ytd.day
    print(f'Running for yx {yx}, year {year}, month {ytd_month}, day {ytd_day}')
################################################################################
    ...
    if success:
            ...
            if cjo is not None and cjo != 'NAY':
                ...
                if odat is not None:
                    ...
                    if idat is not None:
                        ...
                        iport = port_days.index(cdate)
                        ttot = 0.07366
                        if cjo in big_jo_list:
                            iperiod = 'Old'
                        else:
                            iperiod = 'New'
                        involist.append([jo, sh, bk, con, d1, d2, idat.Date, oamt, iperiod])
                        big_jo_list.append(cjo)
                        for tdat in tdata:
        ...
        ws2 = wb.create_sheet(title='Jobs')
        ...
        #Writing Out the Individual Jobs Included
        row = 1
        # write the headers
        headers = ['JO', 'Company', 'Booking', 'Container', 'Date Out', 'Date In', 'Invoice Date', 'Invoice Amount', 'Period']
        flist = [0, 0, 0, 0, 0, 0, 0, money, 0]
        ...
        column_widths = column_wide(headers, involist, [])
        for i, column_width in enumerate(column_widths):
